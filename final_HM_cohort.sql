DROP MATERIALIZED VIEW IF EXISTS siqi_final_HM_cohort CASCADE;
CREATE MATERIALIZED VIEW siqi_final_HM_cohort AS
...
----------------
RCB as (
 select subject_id, hadm_id, icustay_id, itemid as RCB_itemid
  from inputevents_mv
...
----------------
 , RCB_result as (
   ...
   max(CASE WHEN RCB.RCB_itemid >0 then 1 ELSE 0 END)as RCB_1
   from RCB
...
----------------
  ...
  left join RCB_result
   on  final_co.hadm_id = RCB_result.hadm_id
  AND final_co.icustay_id=RCB_result.icustay_id
  AND final_co.subject_id=RCB_result.subject_id
 ...
 where exclusion_1st_care_unit = 0
  AND exclusion_los = 0
  AND exclusion_age = 0
